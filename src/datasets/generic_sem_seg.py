...
class DatasetLabelInfo:
	def __init__(self, label_list):
		...
		self.class_names = ['NULL'] * (max_id+1)
		...
		self.has_trainid = hasattr(self.labels[0], 'trainId')
	...
	def convert_ids_to_colors(self, class_map):
		fr_sh = class_map.shape
		return self.colors_by_id[class_map.reshape(-1)].reshape((fr_sh[0], fr_sh[1], 3))

	def convert_trainIds_to_colors(self, class_map):
		fr_sh = class_map.shape
		return self.colors_by_trainId[class_map.reshape(-1)].reshape((fr_sh[0], fr_sh[1], 3))

	def convert_trainIds_to_ids(self, class_map):
		return apply_label_translation_table(self.table_trainId_to_label, class_map)

	def convert_ids_to_trainIds(self, class_map):
		return apply_label_translation_table(self.table_label_to_trainId, class_map)
