...
class DatasetBase(PytorchDatasetInterface):
	...
	def __init__(self,  b_cache=False):
		# cfg?
...
class ChannelLoaderFileCollection:
	...
	def resolve_template(self, template, dset, frame):
		if isinstance(template, str):
			...
			return template.format(
				...
				frame=frame, 
				fid=frame.get('fid'), 
				fid_no_slash=frame.get('fid', '').replace('/', '__'),
			)
		else:
			# function template
			return template(dset=dset, channel=self, frame=frame, fid=frame.get('fid'))
	...
	def read_value(self, dset=None, **frame):
		path = self.resolve_file_path(dset, frame)
		return self.read_file(path)

	def write_value(self, value, dset=None, **frame):
		path = self.resolve_file_path(dset, frame)
