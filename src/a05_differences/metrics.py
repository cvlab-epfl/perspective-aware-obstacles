...
def binary_confusion_matrix(prob, gt_label, roi=None, levels=NUM_LEVELS, normalize=True):
	if roi is not None:
		prob = prob[roi]
		area = prob.__len__()
	...
	gt_label_bool = gt_label.astype(bool)
	...
	tp, _ = np.histogram(prob_at_true, bins=levels, range=[0, 1])
	tp = np.cumsum(tp[::-1])
...
def contour_inset(mask):
	eroded = cv2.erode(mask.astype(np.uint8), MORPH_KERNEL).astype(bool)
	return eroded != mask
