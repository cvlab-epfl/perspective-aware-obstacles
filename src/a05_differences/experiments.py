...
from ..pipeline.transforms import TrKeepFields
from ..pipeline.transforms_imgproc import TrZeroCenterImgs
from ..pipeline.transforms_pytorch import TrNP, tr_torch_images, image_batch_preproc, image_batch_preproc_undo
...
class ExperimentDifference_Auto_Base(ExperimentDifferenceBin_fakeErr):
	...
	def tr_semseg_errors_to_label(self, semseg_errors, **_):
		...
		# For the 2048x1024 version:
		# new generator saves the out-of-roi 255 into _error.png images
		# If we want to run the 1024x512 version, we should re-generate the _error.png part of the dataset to include it.
		return dict(
	...
	def discrepancy_dataset_init_pipeline(self, use_gt_labels=True, write_orig_label=False):
		...
		self.tr_synthetic_and_save = TrsChain(
			self.tr_alter_labels_and_gen_image,
			# saving as image does not like bool
	...
	def training_visualize_batch(self, **fields):
		if not self.training_visualize_done:
			self.training_visualize_done = True

			training_visualize_batch(
				dir_out = self.train_out_dir,
				out_name = str(self.state['epoch_idx']),
				fields = fields,
			)
...
def training_visualize_batch(dir_out, fields, out_name=''):
	dir_out = Path(dir_out)
	vis_entries = []
	batch_size = 4

	for name in ['image', 'gen_image']:
		img_bchw = fields.get(name)
		if img_bchw is not None:
			
			batch_size = img_bchw.shape[0]

			img_bhwc = image_batch_preproc_undo(img_bchw)

			if img_bhwc.shape.__len__() >= 4:
				vis_entries += list(img_bhwc)
			else:
				vis_entries.append(img_bhwc)

	for name in ['labels_fakeErr_trainIds']:
		label_bhw = fields.get(name)

		if label_bhw is not None:
			label_bhw = label_bhw.detach().cpu().numpy().astype(np.uint8)

			vis_entries += [
				CityscapesLabelInfo.convert_trainIds_to_colors(lb) 
				for lb in label_bhw
			]

	for name in ['semseg_errors_label']:
		label_bhw = fields.get(name)

		if label_bhw is not None:
			label_bhw = label_bhw.detach().cpu().numpy().astype(np.uint8)
			b, h, w = label_bhw.shape

			label_demo = np.zeros((b, h, w, 3), dtype=np.uint8)
			label_demo[label_bhw == 0] = (50, 50, 50)
			label_demo[label_bhw == 1] = (250, 250, 250)
			label_demo[label_bhw == 255] = (210, 0, 210)

			vis_entries += list(label_demo)

	vis_img = image_montage_same_shape(
		vis_entries, 
		num_cols = batch_size, 
		downsample=1,
		border=4,
	...
	imwrite(dir_out / f'training_demo_{out_name}.webp', vis_img)
