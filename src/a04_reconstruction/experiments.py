...
from .transforms import postprocess_gen_img
...
class Pix2PixHD_Generator:
	...
	pix2pixHD_VARIANTS = {
		...
		'0405_nostyle_crop_ctc': f"""
			...
			--resize_or_crop crop --fineSize 384 --batchSize 4
		""",
		'0406_BDD100k_nostyle_ctc_crop': f"""
			--name 0406_BDD100k_nostyle_ctc_crop
			--checkpoints_dir {DIR_EXP}
			--no_instance
			--resize_or_crop crop --fineSize 384 --batchSize 4
		""",
	...
	def load_pix2pixHD_default(cls, variant='0405_nostyle_crop_ctc'):
		...
		p2p_source_dir = (Path(__file__).parent / '../../inpainter').resolve()
		
		p2p_include_dirs = [
			p2p_source_dir,
			p2p_source_dir / 'pix2pixHD', # to make the "util" module loadable
		]

		for d in p2p_include_dirs:
	...
	def tr_generator(self, pred_labels_trainIds, **_):
		"""
		THIS FUNCTION IS WRONG, it provides onehot labels, but mod_pix2pix.inference takes byte-id labels.
		"""
...
class TrPix2pixHD_Generator(TrBase):
	...
	def generate_auto_resolution(self, labels_source, working_resolution_width = 1024):
		"""
		pix2pixHD was trained at 1024x512
		"""

		h, w = labels_source.shape

		b_resize = w != working_resolution_width

		labels = labels_source
		if b_resize:
			scale = working_resolution_width / w

			labels = cv.resize(labels_source, None, fx=scale, fy=scale, interpolation=cv.INTER_NEAREST)

		gen_image = self(labels)['gen_image']

		if b_resize:
			gen_image = cv.resize(gen_image, None, fx=1./scale, fy=1./scale, interpolation=cv.INTER_CUBIC)
