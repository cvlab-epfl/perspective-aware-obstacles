...
def compute_votes(edgelets, model, threshold_inlier=5):
	...
	#print(dot_prod, abs_prod, np.abs(cosine_theta), theta)
...
def ransac_vanishing_point(edgelets, num_ransac_iter=2000, threshold_inlier=5, b_ret_votes=False, b_small_sample=False):
	...
	best_votes: number of inliers supporting the hypothesis
		(if b_ret_votes is True)
	...
	if not b_small_sample:
		first_index_space = arg_sort[:num_pts // 5]
		second_index_space = arg_sort[:num_pts // 2]
	else:
		first_index_space = arg_sort
		second_index_space = arg_sort
	...
	for ransac_iter in range(num_ransac_iter):
		if b_small_sample:
			if num_pts <= 1:
				raise ValueError('There is only 1 point')

			ind1 = np.random.randint(0, num_pts)
			ind2 = np.random.randint(0, num_pts)
			while ind2 == ind1:
				ind2 = np.random.randint(0, num_pts)
		else:
			ind1 = first_index_space[np.random.randint(0, len(first_index_space))]
			ind2 = second_index_space[np.random.randint(0, len(second_index_space))]
...
def compute_homography(image_shape, vp1, vp2, clip=True, clip_factor=3, b_no_size=False):
	...
	if b_no_size:
		return inter_matrix, (0, 0)

	cords = np.dot(inter_matrix, [[0, 0, image_shape[1], image_shape[1]],
								  [0, image_shape[0], 0, image_shape[0]],
								  [1, 1, 1, 1]])
	...
	if clip:
		...
		max_offset = max(image_shape) * clip_factor / 2
...
def compute_homography_and_warp(image_shape, vp1, vp2, clip=True, clip_factor=3):
	"""Compute homography from vanishing points and warp the image.
...
def vis_model_multi(image, vps, show=True, edgelets = None, stride=4, save=None):
	...
	for vp, color in zip(vps, ['r', 'g', 'b', 'm']):
		inliers = compute_votes(edgelets, vp, 10) > 0
		...
		for i in range(0, pts.shape[0], stride):
			xax = [pts[i, 0], vp[0]]
			yax = [pts[i, 1], vp[1]]
			plot.plot(xax, yax, color)
...
def vis_model(image, model, show=True, edgelets = None, stride=4, save=None):
	"""Helper function to visualize computed model."""
	import matplotlib.pyplot as plt
	if edgelets is None:
	...
	for i in range(0, locations.shape[0], stride):
