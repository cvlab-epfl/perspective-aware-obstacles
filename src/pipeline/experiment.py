...
def train_state_init():
	train_state = GrumpyDict(
		...
		best_loss_val = 1e5,
		#histories = {},
		loss_history_train = {}, #indexed by epoch
		loss_history_val = {},
...
class ExperimentBase():
	...
	def __init__(self, cfg=None):
		cfg = EasyDict(cfg or self.cfg) # try reading from static variable
		...
		self.training_visualize_done = False
	...
	def init_net(self, role):
		...
		if role == 'train':
			...
			chk_for_eval_name = self.cfg.get('checkpoint_for_eval', 'best')

			chk = self.load_checkpoint(chk_name=f'chk_{chk_for_eval_name}.pth')
			self.build_net(role, chk=chk)
			self.net_mod.eval()
	...
	def training_epoch_finish(self, epoch_idx, results_avg):
		...
		self.state['loss_history_train'][epoch_idx] = results_avg['loss']
	...
	def val_epoch_finish(self, epoch_idx, results_avg):
		...
		if epoch_idx != 0:
			self.state['loss_history_val'][epoch_idx] = results_avg['loss']
	...
	def run_epoch(self, epoch_idx):
		...
		self.draw_loss_plots()

	def draw_loss_plots(self):
	
		fig, plot = pyplot.subplots(1, 1, figsize=(14, 10))
		
		for split in ('val', 'train'):
			history = self.state[f'loss_history_{split}']
			epochs, values = zip(*history.items())
			
			plot.plot(epochs, values, label=f'loss {split}', marker='.')
		...
		save_name = self.cfg['name'] + '__' + self.state['run_name']
		
		for fmt in ['svg', 'png']:
			fig.savefig(self.workdir / f'{save_name}.{fmt}')
	...
	def training_run(self, b_initial_eval=False):
		name = self.cfg['name']
	...
	@classmethod
	def training_procedure(cls) -> bool:
		print(f'-- Training procesure for {cls.__name__} --')
		exp = cls()
		return exp.training_procedure_on_instance()

	def training_procedure_on_instance(self) -> bool:
		log_config_file(self.workdir / 'training.log')
		
		log.info(f'Starting training job for {self.__class__.__name__}')		
		self.print_cfg()
	...
	def loader_args_for_role(self, role):
		if role == 'train':
			return  dict(
				#sampler = torch.utils.data.sampler.SequentialSampler(range(self.datasets['train'].__len__())),
				shuffle = True,
			...
			return  dict(
				sampler = torch.utils.data.sampler.SequentialSampler(range(self.datasets['val'].__len__())),
				shuffle = False,
