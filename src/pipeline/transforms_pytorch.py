...
def image_batch_preproc(img_bhwc):
	img_bchw_tr = image_to_tensor(img_bhwc).float()
	img_bchw_tr *= (1./255.)
	img_bchw_tr -= IMG_MEAN_DEFAULT
	img_bchw_tr /= IMG_STD_DEFAULT
	
	return img_bchw_tr

def image_batch_preproc_undo(img_bchw_tr):
	img_bchw = img_bchw_tr.detach().cpu().clone()
	img_bchw *= IMG_STD_DEFAULT
	img_bchw += IMG_MEAN_DEFAULT
	img_bchw *= 255.
	img_bhwc = tensor_to_image(img_bchw)
