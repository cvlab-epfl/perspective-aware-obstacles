...
class Pipeline:
	...
	def get_loader(self, dset, threaded_loader=False, force_no_workers=False):
		collate_fn = partial(self.pipline_collate, self.tr_input, self.tr_batch_pre_merge)
		
		if threaded_loader:
			self.loader_class = SamplerThreaded
		else:
			self.loader_class = DataLoader

		loader_args = dict(self.loader_args)
		if force_no_workers:
			loader_args['num_workers'] = 0

		return self.loader_class(dset, collate_fn=collate_fn, **loader_args)

	def apply_tr_output(self, frame):
		frame.apply(self.tr_output)
		return frame

	def execute(self, dset, b_accumulate=True, b_grad=False, b_one_batch=False, b_pbar=True, log_progress_interval=None, short_epoch=None, threaded_loader=False, allow_workers=True):
		loader = self.get_loader(
			dset, 
			threaded_loader=threaded_loader,
			force_no_workers= (not allow_workers) or b_one_batch, # if we do single batch, the iterator will not finish and the workers would not be disposed of
		)
	...
	# def execute_single(self, frame, b_grad=False):
	# 	collate_fn = partial(self.pipline_collate, self.tr_input, self.tr_batch_pre_merge)
